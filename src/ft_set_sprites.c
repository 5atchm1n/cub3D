...
/*   Updated: 2021/04/04 04:12:22 by sshakya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_transform_sprite(t_sprite *s, t_player *player, t_objs *objs, int i)
{
	...
	objs->index = i;
	x = s[objs[i]].x - player->vector.x;
	y = s[objs[i]].y - player->vector.y;
	...
	objs->ty = inv * (-player->vector.dy * x + player->camera.px * y);
}
...
static void		ft_set_sprite_image_buffer(t_mlx *mlx, t_objs *objs, t_world *world, t_sprite *s)
{
	int			x;
	int			d;
	int			texx;
	...
	while (x < objs->endx)
	{
		texx = (int)((256 * (x - (objs->spritew / 2 + objs->hitx)) * SPRITE_W / objs->spritew) / 256);
		if (objs->ty > 0 && x > 0 && x < mlx->res.x && ty < zbuffer[x])
		{
			y = objs->starty;
			while (y < objs->endy)
			{
				d = (y - objs->voffset) * 256 - mlx->res.y * 128 + objs->spriteh * 128;
				texy = ((d * SPRITE_H) / objs->spriteh) / 256;
				colour = world->obj[s[objs->order[objs->index]].id][SPRITE_W * objs->ty + objs->tx];
...
void			ft_cast_sprites(t_cub *game, double *zbuffer)
{
	t_objs		objs;
